...
namespace Core 
{
    namespace UI 
    {
        Input::Input(glm::vec2 position, glm::vec2 dim, float roundness, glm::vec4 color, glm::vec4 textColor, std::shared_ptr<Core::Scene> scene, std::function<void(std::string)> callback)
        {
            std::shared_ptr<InputScript> script = std::make_shared<InputScript>();
            script->SetCallback(callback);
            
            Entity input(scene);
            input.AddComponent<UIComponent>("", glm::vec4(textColor.r, textColor.g, textColor.b, textColor.a));

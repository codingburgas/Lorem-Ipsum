...
namespace Core
{
    namespace UI
    {
        Button::Button(std::string text, glm::vec2 position, glm::vec2 dim, float roundness, glm::vec4 color, glm::vec4 textColor, uint32_t textSize, std::shared_ptr<Core::Scene> scene, std::function<void()> callback)
        {
            std::shared_ptr<ButtonScript> script = std::make_shared<ButtonScript>();
            script->SetCallback(callback);
            
            Entity button(scene);
            button.AddComponent<UIComponent>(text, textColor, textSize);

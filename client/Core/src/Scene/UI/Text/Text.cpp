...
namespace Core 
{
    namespace UI 
    {
        Text::Text(std::string text, glm::vec2 position, glm::vec4 color, uint32_t textSize, std::shared_ptr<Core::Scene> scene)
        {
            Entity textElement(scene);
            
            textElement.AddComponent<UIComponent>(text, color, textSize);
